...
class ADTOFFrameRNN(nn.Module):
    def __init__(
        ...
        self.final_gru_size = final_gru_size
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #     x, _ = gru(x)
        x = self.get_final_gru_output(x)
        x = self.output_layer(x)
        x = torch.sigmoid(x)
        return x
    
    def get_final_gru_output(self, x: torch.Tensor)-> torch.Tensor:
        batch_size, time_steps, freq_bins, channels = x.shape
        x = x.permute(0, 3, 1, 2)
        for cnn_block in self.cnn_blocks:
            x = cnn_block(x)
...
def create_frame_rnn_model(n_bins: int = 168, output_classes: int = 5) -> ADTOFFrameRNN:
